	...
	<div class="col-sm-12">
		<div class="pull-left">
			<shiro:hasPermission name="product:product:add">
				<table:addRow url="${ctx}/product/product/form" title="产品"></table:addRow><!-- 增加按钮 -->
			</shiro:hasPermission>
			<shiro:hasPermission name="product:product:edit">
			    <table:editRow url="${ctx}/product/product/form" title="产品" id="contentTable"></table:editRow><!-- 编辑按钮 -->
			</shiro:hasPermission>
	...
	<table id="contentTable" class="table table-striped table-bordered table-hover table-condensed dataTables-example dataTable">
		<thead>
			<tr>
				...
				<th  class="sort-column iteration">迭代</th>
		...
		<c:forEach items="${page.list}" var="product">
			<tr>
				...
				<td ><a  href="#" onclick="openDialogView('查看产品', '${ctx}/product/product/form?id=${product.id}','800px', '500px')">
					${product.id}
				</a></td>
				<td >
					${product.name}
				...
				<td >
					${fns:getDictLabel(product.status, 'pro_status', '')}
				</td>
				<td >
					${product.iteration}
				</td>
				...
				<td>
					<shiro:hasPermission name="product:product:view">
						<a href="#" onclick="openDialogView('查看产品', '${ctx}/product/product/form?id=${product.id}','800px', '500px')" class="btn btn-info btn-xs" ><i class="fa fa-search-plus"></i> 查看</a>
					</shiro:hasPermission>
					<shiro:hasPermission name="product:product:edit">
    					<a href="#" onclick="openDialog('修改产品', '${ctx}/product/product/form?id=${product.id}','800px', '500px')" class="btn btn-success btn-xs" ><i class="fa fa-edit"></i> 修改</a>
    				</shiro:hasPermission>
    				<shiro:hasPermission name="product:product:del">
						<a href="${ctx}/product/product/delete?id=${product.id}" onclick="return confirmx('确认要删除该产品吗？', this.href)"   class="btn btn-danger btn-xs"><i class="fa fa-trash"></i> 删除</a>
					</shiro:hasPermission>
				</td>
			</tr>
		</c:forEach>
